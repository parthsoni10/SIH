"""
SENTINEL V3.3 — Stage: Machine-Readable Integrity (QR / barcode)
==================================================================
Evaluates machine-readable symbols (QR codes) for Reed-Solomon codeword
validity and demographic payload cross-checking.

Mandatoriness is derived from `DocumentSpec.qr_expected_side` dynamically,
covering all document types without hardcoded lists.
"""
...
def analyse_qr(bgr: np.ndarray,
               document_type_or_spec: str | Any,
               ocr_fields: dict[str, Any] | None = None,
               side: str = "FRONT") -> QRResult:
    """Accepts either a `DocumentSpec` object or a `document_type` string."""
    r = QRResult()
    from .document_spec import DocumentSpec, get_spec
    if isinstance(document_type_or_spec, DocumentSpec):
        spec = document_type_or_spec
    elif isinstance(document_type_or_spec, str):
        spec = get_spec(document_type_or_spec)
    else:
        spec = get_spec("Other")
