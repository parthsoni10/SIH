...
def decide(*,
    ...
    if q < T["quality_floor"] or image_quality.get("quality_too_low_for_forensics"):
        ...
        d.confidence = 0.30
        rc += ["IMAGE_QUALITY_BELOW_FORENSIC_FLOOR", "QUALITY_TOO_LOW_FOR_FORENSICS", "AUTHENTICITY_NOT_ESTABLISHED"]
    ...
    if dv >= T["doc_validity_genuine"] and fusion.anchor_present and ai >= 0.50:
        ...
        d.confidence = 0.40
        rc += ["STRUCTURE_VALID_BUT_PROVENANCE_ANOMALOUS", "AUTHENTICITY_NOT_ESTABLISHED"]
        rc += [f"STRONG_FAMILY_{f}" for f in fusion.strong_families]
    ...
    if structurally_sound:
        rc.append("DOCUMENT_STRUCTURE_VALID")
    ...
    if T["ai_low"] < ai < T["ai_strong"]:
        ...
        rc.append("AI_EVIDENCE_INCONCLUSIVE")
    if low_tamper:
        rc.append("TAMPERING_EVIDENCE_LOW")
    ...
    rc.append("AUTHENTICITY_NOT_ESTABLISHED")
