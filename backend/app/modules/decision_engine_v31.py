...
================================
Hierarchical 9-gate decision engine supporting `INCOMPLETE_SUBMISSION`.

Gate ordering (first match wins):
  1. blacklist
  2. corroborated alteration (catches erased mandatory artwork)
  3. structural invalidity
  4. incomplete submission (catches front-only two-sided docs)
  5. required-field gaps
  6. capture quality
...
def decide(*,
    ...
    ai = float(getattr(fusion, "ai_probability", 0.0))
    ai_corroborated = bool(getattr(fusion, "corroborated", False))
    genuine_ev = float(getattr(fusion, "genuine_evidence", 0.0))

    alt = float(getattr(alteration, "probability", 0.0))
    alt_corroborated = bool(getattr(alteration, "corroborated", False))
    d.suspect_regions = list(getattr(alteration, "suspect_regions", []))

    dv = float(getattr(structure, "validity_score", 0.0))
    ...
    if alt_strong and ai_strong:
        ...
        rc += [f"ALTERATION_CUE_{c.upper()}" for c in getattr(alteration, "strong_cues", [])]
    ...
    if alt_strong:
        ...
        rc += [f"ALTERATION_CUE_{c.upper()}" for c in getattr(alteration, "strong_cues", [])]
        for a in getattr(alteration, "cues", {}).get("anchor_removal", {}).get("detail", {}).get("missing_anchors", []):
    ...
    if not getattr(structure, "submission_complete", True):
        ...
        d.officer_action = ("Document not fully presented. Capture the "
                            + " and ".join(s.lower() for s in getattr(structure, "sides_missing", []))
                            + " side and resubmit. No verdict is admissible until then.")
        return d

    # ---- Gate 5: required-field gaps -------------------------------------
    missing_req = getattr(structure, "missing_required", [])
    role_conflicts = getattr(structure, "role_conflicts", [])
    if missing_req or role_conflicts:
        ...
        d.officer_action = ("Mandatory fields could not be read or failed "
                            ...
                            + ", ".join(missing_req + role_conflicts))
        return d
    ...
    if ai_strong:
        ...
        rc += [f"STRONG_FAMILY_{f}" for f in getattr(fusion, "strong_families", [])]
        d.officer_action = "Document is a synthetic render. Seize and report."
        return d

    # ---- Gate 8: contradiction -------------------------------------------
    anchor_present = getattr(fusion, "anchor_present", True)
    if dv >= T["doc_validity_genuine"] and anchor_present and ai >= 0.50:
        d.status = "MANUAL_REVIEW"
    ...
    sound = getattr(structure, "structurally_valid", False)

    route_a = sound and low_ai and low_alt and genuine_ev >= T["genuine_evidence_min"]
    route_b = (sound and low_ai and low_alt
               and getattr(fusion, "available_family_count", 0) >= 2
               and getattr(alteration, "available_cue_count", 0) >= 2
    ...
    if route_a or route_b:
        ...
        d.confidence = round(min(0.98, 0.55 + 0.30 * getattr(fusion, "ai_confidence", 0.0)
                                 + 0.15 * getattr(alteration, "confidence", 0.0)), 3)
        ...
        if getattr(alteration, "background_flattened", False):
    ...
    d.status = "MANUAL_REVIEW"
    d.evidence_state = "INSUFFICIENT"
    d.confidence = round(0.30 + 0.30 * getattr(fusion, "ai_confidence", 0.0), 3)
    if getattr(fusion, "available_family_count", 0) < 2:
        rc.append("INSUFFICIENT_INDEPENDENT_EVIDENCE_FAMILIES")
    if getattr(alteration, "available_cue_count", 0) < 2:
