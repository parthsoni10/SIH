"""
SENTINEL V3.3 — Field Role Resolution & Structural Completeness
================================================================
Fixes three distinct edge cases:
1. Date role confusion (via bounding box label proximity & vertical orientation check)
2. Internal checksum evaluation via checksum.verify() for Aadhaar, PAN, Passport, Visa, DL, and Other
3. Single-side vs two-sided submission tracking (sides_required check yielding INCOMPLETE_SUBMISSION)
"""
...
def _box_distance(a, b) -> float:
    ax = (a.get("x0", 0.0) + a.get("x1", 1.0)) / 2.0
    ay = (a.get("y0", 0.0) + a.get("y1", 1.0)) / 2.0
    bx = (b.get("x0", 0.0) + b.get("x1", 1.0)) / 2.0
    by = (b.get("y0", 0.0) + b.get("y1", 1.0)) / 2.0
    return ((ax - bx) ** 2 + (ay - by) ** 2) ** 0.5


def _is_vertical(box) -> bool:
    w = max(box.get("x1", 1.0) - box.get("x0", 0.0), 1e-6)
    h = max(box.get("y1", 1.0) - box.get("y0", 0.0), 1e-6)
    return (h / w) > 2.5
...
def resolve_name(lines: list[dict], spec: DocumentSpec) -> ResolvedField:
    ...
    for ln in lines:
        ...
        if dob_y is not None and ln.get("y1", 1.0) <= dob_y:
            score += 0.5 * (1.0 - min(abs(dob_y - ln.get("y1", 1.0)), 0.3) / 0.3)
        if score > best_score:
...
def resolve_structure(*,
    ...
    # ---- checksum: computed internally -----------------------------------
    algo = (spec.checksum or "NONE").upper()
    r.checksum_algorithm = algo
    if checksum_valid is not None:
        r.checksum_applicable = algo != "NONE"
        r.checksum_valid = checksum_valid
        r.checksum_detail = "passed from legacy pipeline"
    elif algo in ("MRZ_TD3", "MRZ_TD1"):
    ...
    if r.checksum_applicable and r.checksum_valid is False:
        r.reasons.append(f"CHECKSUM_FAILED_{algo}")
    elif r.checksum_applicable and r.checksum_valid is None:
